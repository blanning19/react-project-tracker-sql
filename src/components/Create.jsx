...
const Create = () => {
  ...
  const GetData = async () => {
    try {
      const pmRes = await FetchInstance.get("projectmanager/");
      setProjectmanager(pmRes.data);

      const empRes = await FetchInstance.get("employees/");
  ...
  const submission = async (data) => {
    ...
    try {
      const res = await FetchInstance.post("project/", payload);
      console.log("POST /project/ OK", res.status, res.data);
      navigate(`/`);
    } catch (err) {
      // With our fetch wrapper, server JSON is on err.data (not err.response.data)
